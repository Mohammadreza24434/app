...
# ==================== تم حرفه‌ای — از همون اول اعمال میشه ====================
...
<style>
    ...
    .pollutant-value {font-size: 3rem; font-weight: 900; color: white !important;}
    ...
    .license-box {background: rgba(255,255,255,0.1); backdrop-filter: blur(20px); padding: 50px; border-radius: 25px;
                  text-align: center; border: 1px solid rgba(255,255,255,0.2); max-width: 500px; margin: 0 auto;}
</style>
...
# ==================== صفحه اصلی ====================
    ...
    if st.button("Get Live Report", type="primary", use_container_width=True):
        with st.spinner("Loading real-time data..."):
            ...
            forecast_aqi = [max(calc_aqi(x['components']['pm2_5'], pm25_bp),
                               calc_aqi(x['components']['pm10'], pm10_bp),
                               calc_aqi(x['components']['o3']*1000, o3_bp),
                               calc_aqi(x['components']['no2'], no2_bp)) for x in forecast]
